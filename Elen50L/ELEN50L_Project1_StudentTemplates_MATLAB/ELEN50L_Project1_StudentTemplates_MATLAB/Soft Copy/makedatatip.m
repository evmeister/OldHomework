...
% Get the current MATLAB release being used   --> Layne D. Orr, 4/10/2015
...
for n = 1:size(index,1)
  ...
  elseif isempty(Z)
    if strcmp(matlab_rel, '2014b')
        pos = [X(index(n)) Y(index(n)) 0]; % Required for 2014b
    else
  ...
  % Specify data cursor properties
    if ~strcmp(matlab_rel, '2014b') % Does not work in 2014b release
        if isImage
