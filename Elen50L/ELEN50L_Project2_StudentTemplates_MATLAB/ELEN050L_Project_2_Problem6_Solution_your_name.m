%% ELEN 050L (Electric Circuits I): Project 2, Problem 6, Evan Eberhardt, Ryan Meyer
%
% <<ELEN050L_Project_2_figure_02.png>>
%
% <<ELEN050L_Project_2_Problem_06.png>>
%
...
A_0_target = 0.01;      w_0 = 0;           % Amplitude (unitless) at rad/sec
A_1_target = 0.9;      w_1 = 10000;           % Amplitude (unitless) at rad/sec
...
den = 100*(sqrt(1+(w.*R2.*C2).^2));
num = 1*(sqrt(1+(w.*R1.*C1).^2));
...
set(fignum, 'Name', ['Problem 6 A(w)']);      % Name the figure
semilogx(w, F); grid on;
ylabel('Amplitude (dB)');
xlabel('Frequency (rad/s)');
axis([min(w), max(w), -45, 15]);
title ('Problem 6 Solution');

%% Problem 7 actual design verification point 1
%

% Calculate the first target design point A(w).
den = 100*(sqrt(1+(w_0*R2.*C2).^2));
num = 1*(sqrt(1+(w_0*R1.*C1).^2));
% In this case, the numerator is a function of the frequency.
A_0_design = num./den;
...
den = 100*(sqrt(1+(w_1*R2.*C2).^2));
num = 1*(sqrt(1+(w_1*R1.*C1).^2));
% In this case, the numerator is a function of the frequency.
A_1_design = num./den;
...
% Import Excel measurements for Problem 6
[freq_meas,Vg_meas_6,Vo_meas_6] = importfile_problem6...
    ('ELEN050L_Project_2_Problem_6_Measured_Results.xlsx','Sheet1',2,80);

% Define measured values for the circuit elements.
R1_meas = 20000;                  % Ohms
R2_meas = 200;                  % Ohms
C1_meas = 0.000001;                  % Farads
C2_meas = 0.000001;                  % Farads
Vcc_p_meas = 15;               % Volts
Vcc_n_meas = -15;               % Volts

% Calculate A_meas(w) using the measured data.
A_meas = Vo_meas_6./Vg_meas_6;
...
fignum = fignum+1; figObj = figure(fignum);   % Establish a figure number
set(fignum, 'Name', ['Problem 6 A_meas(w)']); % Name the figure
semilogx(w_meas, F_meas); grid on;
ylabel('Amplitude (dB)');
xlabel('Frequency (rad/s)');
axis([min(w_meas), max(w_meas), -45, 15]);
title ('Problem 6 Solution (Measured)');
